...
<div class="form-center">
    <form action="{{ route('clients.update', $client->id) }}" method="POST" class="company-forms form-control form-control-sm p-3">
        @csrf
        @method('PATCH')
        ...
        <div class="mb-2">
            ...
            <input type="text" class="form-control @error('cpf') is-invalid @enderror" id="cpf" name="cpf" value="{{ old('cpf', $client->cpf) }}" required>
            @error('cpf')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
